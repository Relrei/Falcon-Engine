...
- VSE の再生を GPU で: 動画は GPU でデコード(NVDEC)し、色の変換と表示まで CPU のメモリに降ろさずに GPU の中で済ませます(NVIDIA の GPU がある時・既定で有効)。10bit の動画もプレビューでは 8bit で読むので軽く、画像の連番(PNG など)は複数の処理で先読みします
- DLSS でアニメーションを書き出す時にメモリが尽きて落ちる不具合を直しました(書き出しの間だけ永続データを自動で使います)
- VSE のヘッダーに秒/フレーム表示・チャンネル欄の開閉・プレビュー解像度・チャンネル番号を常時表示(今まではメニューの奥)
- VSE のチャンネルを上下の矢印ボタンで前後入れ替え(隣のチャンネルと中身をまるごと交換)
- プレビューでハンドルをドラッグしてクロップ([BL Easy Crop](https://github.com/usrname0/BL_EasyCrop) をサードパーティ製として同梱・GPL-3.0-or-later)
- 追加した映像も自動でスムーズに再生するプロキシの仕組みが、16bit の画像連番でも正しく動くように修正
- 再生中に Strip を削除すると落ちる不具合・再生開始直後だけ GPU が正しく使われない不具合を修正

  ★上の5件の使い方は画像付きで [docs/vse-2026-09-23-updates.md](docs/vse-2026-09-23-updates.md) にまとめてあります。
