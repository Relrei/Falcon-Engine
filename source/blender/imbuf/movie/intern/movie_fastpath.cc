  ...
  if ((rd->ffcodecdata.flags & FFMPEG_NO_HARDWARE_ENCODER) == 0 && hw_takes_pix_fmt) {
    const char *hw_name = (par->codec_id == AV_CODEC_ID_H264)  ? "h264_nvenc" :
                          (par->codec_id == AV_CODEC_ID_H265) ? "hevc_nvenc" :
                                                                 nullptr;
    if (hw_name != nullptr) {
      hw_codec = avcodec_find_encoder_by_name(hw_name);
    }
  }
  const AVCodec *cpu_codec = avcodec_find_encoder(par->codec_id);
  if (hw_codec == nullptr && cpu_codec == nullptr) {
    FAIL("No encoder for the source codec; the partial groups cannot be re-encoded");
  }

  /* ★NVENC が FFmpeg に**入っている**ことと、この機械で**開ける**ことは別。
   * NVIDIA の GPU が無い機械(AMD / Intel・サンドボックスで /dev/nvidia* を見せない時)では
   * `avcodec_open2` が落ちる。ここで諦めると **fast path 全体が黙って降りて**
   * 通常の書き出し(全コマの焼き直し)になる(2026-09-17 bwrap で実測:
   * 「Cannot open the encoder for the partial groups」)。通常の経路の
   * `encoder_opens` と同じく、開けなければ CPU の符号化器で開き直す。 */
  const AVCodec *candidates[2] = {hw_codec, cpu_codec};
  for (const AVCodec *enc_codec : candidates) {
                                 ...
                                 int reason_maxncpy)
{
  ...
  /** 復号器から出てきたコマを、範囲内なら焼き直して書く。 */
  auto take_decoded_frames = [&]() {
  ...
  /* ★素材の終わりまで読んだ時は、**復号器の中にまだコマが残っている**。
   *
   * B フレームのある素材では、復号器が並べ替えのぶん(`has_b_frames`)だけコマを
   * 抱えたままになる。区間が素材の途中で終わる時は**次のパケットが押し出して**
   * くれるので表に出るが、**末尾まで読み切った時は押し出す物が無い**。
   * 空送り(NULL パケット)で吐き出させないと、そのまま消える。
   *
   * 実測(48コマの素材を末尾まで書き出す): bf=0/1/2/3/4 で 0/1/2/2/2 コマ落ちた
   * = **落ちた数が has_b_frames と完全に一致**。しかも info には頼んだ枚数を
   * 書くので、**出来たファイルを数えるまで気づけない**。
   * 符号化器の空送り(すぐ下)は元からあったが、復号器の側が抜けていた。
   *
   * ⚠ ここで復号器は「出し切った」状態になる。次の焼き直し区間は頭で
   * `avcodec_flush_buffers(re.dec)` を通るので、使い回しても問題ない。 */
  if (ok && !done) {
    avcodec_send_packet(re.dec, nullptr);
    take_decoded_frames();
  }
