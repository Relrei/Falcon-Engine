...
int gpu_preview_ahead_frames()
{
  static const int n = []() {
    ...
    if (env == nullptr || env[0] == '\0') {
       ...
       *
       * ★2026-09-22 既定を 8 に変えた(段 3 = NVDEC が繋がった後の実測)。
       * 0 のままだと先読みが NVDEC の絵を CPU へ降ろして色変換し、表示側もそれを使うので、
       * GPU は復号しかしない。1080p 10bit HEVC の区間: 0 = 39fps・CPU 230% / 8 = 53fps・CPU 18%。
       * PNG の区間は 0 と 8 で同じ(約 10fps・復号は CPU しか無い)。GPU 入切・コマ飛ばし入りで 3/3。
       * 戻す口: FALCON_VSE_GPU_AHEAD=0 */
      return 8;
    }
