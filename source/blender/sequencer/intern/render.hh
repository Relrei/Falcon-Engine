 ...
 * touching the task pool at all, when nothing was ever read ahead
 * (BL_VSE_PREFETCH_N=0, or no render has run yet: read-ahead only runs
 * while rendering).
 */
 ...
 * touching anything, when nothing was ever read ahead (BL_VSE_PREFETCH_N=0,
 * or no render has run yet: read-ahead only runs while rendering).
 */
