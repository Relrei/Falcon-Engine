 ...
 *
 * ★Falcon Engine の既定は**オン**(チャンネル 1 が最上段・AviUtl と同じ向き)。
 * 本家 Blender と同じ「下から上」に戻すには `FALCON_VSE_FLIP_CHANNELS=0`。
 * (作者の普段の起動は以前からこれを 1 にしていた。公開・デモの建てでも同じ向きにそろえる。)
 *
 * Controlled by the `FALCON_VSE_FLIP_CHANNELS` environment variable. Read once via
 * `getenv()` on first use and cached in a function-local
