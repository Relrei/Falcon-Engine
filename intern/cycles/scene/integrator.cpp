...
static string falcon_confine_cache_path(const string &requested)
{
  ...
  LOG_WARNING << "Falcon: cache path " << requested << " is outside " << dir << ", using "
              << confined << " instead "
              << "(FALCON_SHARC_ALLOW_ABSOLUTE_CACHE=1 restores the previous behavior)";
  return confined;
}
#endif
...
void Integrator::device_update(Device *device, DeviceScene *dscene, Scene *scene)
{
   ...
   * cache allocation and (via falcon_sharc_active) no in-kernel cache lookup. */
  {
    ...
    if (pr > FALCON_PHOTON_RADIUS_MAX) {
      LOG_WARNING << "Falcon: caustic spread " << pr << " exceeds the limit "
                  << FALCON_PHOTON_RADIUS_MAX << ", clamped "
                  << "(baking with this value never returns from the GPU)";
    }
    ...
    kintegrator->falcon_lt_photons = 0.0f;
    kintegrator->falcon_photon_all_lights = 0;
    ...
    if (getenv("FALCON_LIGHTTRACE") && strcmp(getenv("FALCON_LIGHTTRACE"), "1") == 0) {
      ...
      /* ★2026-09-06 S1: 打ち消す相手を「撃った光子の数」にする。
       * FALCON_PHOTON_N は総光子数なので、画素数で割って「画素あたりの
       * 光子数」を渡す(今の呼び出しでは = SPP なので絵は変わらない)。
       * 割り算を倍精度でやってから float にするのは、総数(1e9 級)を
       * float に落としてから割ると最後の桁がずれて画素一致が崩れるため。 */
      const char *nph_env = getenv("FALCON_PHOTON_N");
      /* width/height は private なので full_width/full_height を使う。
       * LT は境界レンダーを許さない(splat が1タイル前提)ので、この2つは
       * kernel_data.cam.width/height と同じ値になる。ずれれば G1 の画素
       * 一致がすぐ落ちる。 */
      const double npx = scene->camera ? (double)scene->camera->get_full_width() *
                                             (double)scene->camera->get_full_height() :
                                         0.0;
      kintegrator->falcon_lt_photons = (nph_env && npx > 0.0) ?
                                           (float)(atof(nph_env) / npx) :
                                           0.0f;
    ...
    /* ★2026-09-06 S4: FALCON_LT_ONE_PASS=1 で、1回のレンダーで全部の灯から
     * 撒く(今までは灯ごとに別レンダー→ホストで足し算)。灯の選択と
     * 灯ごとの出力の計算はカーネル側でやる(ホストとデバイスで灯の並びが
     * 一致している保証が無いので、合計をホストで作って渡すと食い違う)。
     * ホストがやるのは「発射窓」だけで、そこにはランプの向きが要るので
     * 代表の灯として SUN を優先して拾う(SUN は普通1つ)。 */
    const char *lt_one_env = getenv("FALCON_LT_ONE_PASS");
    const bool lt_one_pass = (kintegrator->falcon_lighttrace != 0) && lt_one_env &&
                             strcmp(lt_one_env, "1") == 0;
    if (photon_mode && strcmp(photon_mode, "bake") == 0) {
      for (Object *ob : scene->objects) {
        if (ob->get_geometry() && ob->get_geometry()->is_light()) {
          ...
          if (l->get_is_enabled()) {
            const bool is_sun = (l->get_light_type() == LIGHT_SUN);
            if (photon_light == nullptr || (lt_one_pass && is_sun)) {
              photon_light = l;
              photon_light_ob = ob;
            }
            if (!lt_one_pass || is_sun) {
              break;
            }
          }
        }
    ...
    if (photon_light || world_radiance > 0.0f) {
      ...
      if (lt_one_pass && !kintegrator->falcon_photon_is_world) {
        /* 灯ごとの出力(SPOT の Omega/4pi・SUN の E*cos*A・POINT/AREA の watts)
         * はカーネルが KernelLight から自分で作る。ここで渡すのは
         * 「1 W あたり・光子1個あたり」の係数だけ。 */
        kintegrator->falcon_photon_all_lights = 1;
        kintegrator->falcon_photon_flux = (float)(gain / n_photons);
        LOG_INFO << "Falcon Photon: all lights in one pass (num_lights is on the kernel side)";
      }
    ...
    if (!map_path.empty()) {
      ...
      if (f) {
            ...
            dims[1] == render_height)
        {
          ...
          LOG_WARNING << "Falcon DAS: " << map_path << " is " << dims[0] << "x" << dims[1]
                      << ", which does not match this render's " << render_width << "x"
                      << render_height << ", not used";
        }
        fclose(f);
      }
    }
