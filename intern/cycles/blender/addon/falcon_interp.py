...
def find_rife():
    ...
    return None, "%s not found (looked in: %s)" % (RIFE_EXE, ", ".join(tried))
...
def interp_pairs(allf, keep, cut, step):
    ...
    for a, b in zip(ks, ks[1:]):
        ...
        for c in cut:
            if a < c <= b:
                raise RuntimeError(
                    "cut frame %d lies between rendered frames %d..%d: the plan is broken"
                    % (c, a, b))
        f = a + step
        while f < b:
...
def _gate(scene):
    ...
    if r.is_movie_format:
        _log("off: movie output is not supported (image sequences only)")
        return 1
    if r.image_settings.file_format != 'PNG':
        _log("off: the output is %s; only PNG sequences are supported" % r.image_settings.file_format)
        return 1
    if not r.use_file_extension:
        _log("off: with \"File Extensions\" turned off the output files cannot be found")
        return 1
    ...
    if exe is None:
        ...
        return 1
    if model is None:
        _log("off: the %s model (%s) is not next to %s" % (RIFE_EXE, RIFE_MODEL, os.path.dirname(exe)))
        return 1
    return factor
...
def render_complete(scene, _depsgraph=None):
    ...
    if real != sorted(keep):
        _log("nothing to do: %d frames were rendered, %d were planned "
             "(a single-frame render, or it was stopped midway)" % (len(real), len(keep)))
        return

    exe, model = find_rife()
    if exe is None:
        _log("stopped: %s" % model)
        return
    ...
    if missing:
        _log("stopped: %d rendered frames are missing (first 5: %s)" %
             (len(missing), missing[:5]))
        return

    _log("x%d: making %d frames with %s" % (factor, len(pairs), os.path.basename(exe)))
    t0 = time.time()
    made = 0
    ...
    _log("x%d: made %d frames (%.1f s, %.3f s per frame)" %
