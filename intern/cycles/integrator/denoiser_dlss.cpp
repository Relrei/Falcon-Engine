...
/* Falcon plugin folders handed over by the Cycles add-on (see `falcon_plugins.py`). */
static thread_mutex dlss_plugin_mutex;
static vector<string> dlss_plugin_paths;
/* Falcon: DLSS switched off by the add-on (`DLSSDenoiser::set_enabled`). */
static bool dlss_enabled = true;
...
/* Plugin folders first, then the application directory (the NGX default). */
static vector<std::wstring> dlss_search_paths()
{
  vector<std::wstring> paths;
  {
    const thread_scoped_lock lock(dlss_plugin_mutex);
    for (const string &path : dlss_plugin_paths) {
      paths.push_back(dlss_to_wide(path));
    }
  }
  paths.push_back(dlss_to_wide(path_get()));
  return paths;
}

void DLSSDenoiser::set_plugin_paths(const vector<string> &paths)
{
  {
    const thread_scoped_lock lock(dlss_plugin_mutex);
    if (paths == dlss_plugin_paths) {
      return;
    }
...
vector<string> DLSSDenoiser::get_plugin_paths()
{
  const thread_scoped_lock lock(dlss_plugin_mutex);
  return dlss_plugin_paths;
}

void DLSSDenoiser::set_enabled(const bool enabled)
{
  {
    const thread_scoped_lock lock(dlss_plugin_mutex);
    if (enabled == dlss_enabled) {
      return;
    }
    ...
    : DenoiserGPU(denoiser_device, params)
{
  ...
  const std::wstring user_path = dlss_to_wide(path_user_get());

  /* Plugin folders + the application directory. */
  const vector<std::wstring> search_paths = dlss_search_paths();
  ...
  feature_info.PathListInfo.Length = (unsigned int)app_paths.size();
...
bool DLSSDenoiser::is_device_supported(const DeviceInfo &device)
{
  ...
  /* Falcon plugin folders: without `FeatureInfo` the discovery only looks in the application
   * directory, and a runtime placed in a plugin folder would be reported as unsupported. With no
   * plugin folder this stays null (unchanged behavior). */
  const bool have_plugin_paths = !get_plugin_paths().empty();
  const vector<std::wstring> search_paths = have_plugin_paths ? dlss_search_paths() :
                                                                vector<std::wstring>();
  vector<const wchar_t *> search_path_ptrs;
  for (const std::wstring &path : search_paths) {
    search_path_ptrs.push_back(path.c_str());
  }
  NVSDK_NGX_FeatureCommonInfo common_info = {};
  if (have_plugin_paths) {
    common_info.PathListInfo.Path = search_path_ptrs.data();
    common_info.PathListInfo.Length = (unsigned int)search_path_ptrs.size();
    discovery_info.FeatureInfo = &common_info;
  }
                                                    ...
                                                    const DenoisePass &pass)
{
   ...
   * the input pixel it came from: the kernel walks the denoised (full frame)
   * grid, writes the colour RR produced, and then reads the render buffer at
   * int(x / upscale), int(y / upscale) for the alpha it copies over and for the
   * albedo it re-modulates with. So it has to be the *total* ratio between the
   * two buffers, not the panel's upscale quality.
   *
   * Those two were the same thing until playback started rendering smaller
   * (RenderScheduler::playback_upscale_factor, 2026-08-31): from then on the
   * render buffer is up to 2.9x smaller than the frame while params_ still says
   * what the panel asked for, and the same single mismatch produced both of the
   * pictures reported on 2026-09-10 --
   *
   *   panel = None -> 1.0 read out of a 1/2.9 buffer: the index runs off the end
   *                   of the small buffer after the first ~1/8 of the frame, so
   *                   alpha comes from never-written memory (zero) and the
   *                   viewport shows the bottom band and nothing else;
   *   panel = 2.0  -> 2.0 read out of a 1/2.9 buffer: every row lands on the
   *                   wrong input row, and the error grows upwards = the
   *                   horizontal streaks.
   *
   * The ratio is taken from the buffers themselves (max of the two axes, so
   * int(x/upscale) can never step past the last input pixel -- scale_buffer_params
   * truncates each axis separately), and only ever raises the panel's value, so
   * every path that is not shrinking (final renders, and the viewport when it is
   * not playing) passes exactly what it passed before.
   * FALCON_DLSS_POSTPROCESS_PANEL_UPSCALE=1 goes back to the panel's value. */
  static const bool panel_upscale = getenv("FALCON_DLSS_POSTPROCESS_PANEL_UPSCALE") &&
                                    atoi(getenv("FALCON_DLSS_POSTPROCESS_PANEL_UPSCALE")) != 0;
  float total_upscale = params_.upscale_factor;
  if (!panel_upscale && context.buffer_params.width > 0 && context.buffer_params.height > 0) {
    const float ratio_x = float(buffer_params.width) / float(context.buffer_params.width);
    const float ratio_y = float(buffer_params.height) / float(context.buffer_params.height);
    const float ratio = (ratio_x > ratio_y) ? ratio_x : ratio_y;
    if (ratio > total_upscale + 0.01f) {
      total_upscale = ratio;
    }
  }

  if (getenv("FALCON_DLSS_DEBUG") && total_upscale != params_.upscale_factor) {
    static float last_reported = 0.0f;
    if (total_upscale != last_reported) {
      last_reported = total_upscale;
      fprintf(stderr,
              "[dlss] postprocess upscale: panel=%.3f total=%.3f (in %dx%d -> out %dx%d)\n",
...
bool DLSSDenoiser::denoise_run(const DenoiseContext &context, const DenoisePass &pass)
{
  ...
  /* ★A freshly (re)created feature has no history at all -- so telling the
   * runtime "keep using yours" (Reset=0) is always wrong there: it blends
   * whatever happens to be in its internal history buffers into a picture that
   * has nothing to blend with, and artefacts appear out of nowhere.
   *
   * This is what stopping playback looks like (2026-09-10, reported): playback
   * renders at a reduced internal resolution (RenderScheduler::
   * playback_upscale_factor), so leaving playback changes the input size and
   * ensure_denoiser() releases and rebuilds the feature with have_history_ =
   * false. The frame did not change and the sample count did not restart, so
   * frame_transition is false and is_reset stayed 0 on that very first
   * evaluation. Entering playback has the same hole, but the picture is moving
   * there so the garbage is washed out within a frame; on the way out the image
   * is static, so it sits on screen.
   *
   * have_history_ is only ever true after an evaluation succeeded, so this is
   * exactly "we know there is nothing to carry". FALCON_DLSS_FRESH_RESET=0
   * restores the 2026-09-09 behaviour. */
  static const bool fresh_reset = getenv("FALCON_DLSS_FRESH_RESET") ?
                                      atoi(getenv("FALCON_DLSS_FRESH_RESET")) != 0 :
                                      true;
  if (fresh_reset && !have_history_) {
    is_reset = true;
  }
