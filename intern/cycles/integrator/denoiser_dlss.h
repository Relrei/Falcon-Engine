...
class DLSSDenoiser : public DenoiserGPU {
 public:
  ...
  /* Falcon plugin folders: extra directories where NGX looks for the DLSS runtime, in addition
   * to the application directory. Clears the cached support result and asks the CUDA/OptiX
   * device lists to be detected again, so a runtime placed or removed at run time is seen. */
  static void set_plugin_paths(const vector<string> &paths);
  static vector<string> get_plugin_paths();

  /* Falcon: DLSS turned on or off (by the DLSS add-on, see `falcon_plugins.py`). While off no device
   * reports DLSS support, also after NGX has loaded the runtime once in this process (its discovery
   * keeps reporting a runtime it has loaded, whatever search paths it is given). On by default. */
