...
/* ★2026-09-06 S4: 1つの KernelLight が「光子として撒ける出力」。
 * 灯ごとに別レンダーしていた時はホスト(scene/integrator.cpp)が灯の種類ごとに
 * この式を作って falcon_photon_flux に畳んでいた。1パスにすると灯を選ぶのが
 * カーネル側になるので、同じ式をここに持つ。
 * ★ホストで合計を作って渡さないのは、ホストの scene->objects の並びと
 *   デバイスの lights[] の並びが一致している保証がないため。 */
ccl_device_inline float falcon_photon_light_power(KernelGlobals kg,
                                                  const ccl_global KernelLight *kl)
{
  const float watts = (kl->strength[0] + kl->strength[1] + kl->strength[2]) * (1.0f / 3.0f);
  if (!(watts > 0.0f)) {
    return 0.0f;
  }
  if (kl->type == LIGHT_POINT || kl->type == LIGHT_AREA) {
    return watts;
  }
  if (kl->type == LIGHT_SPOT) {
    /* Blender の spot は「点光源 W/(4 pi) を円錐で切った物」。円錐から出る
     * 出力は W * Omega/(4 pi) = W * (1 - cos_half) / 2。 */
    return watts * 0.5f * (1.0f - kl->spot.cos_half_spot_angle);
  }
  if (kl->type == LIGHT_SUN) {
    /* 太陽の強さは放射照度。ホストが決めた発射窓(矩形)を通る出力は
     * E * cos(天頂角) * 面積。 */
    const float3 nd = normalize(make_float3(kl->co.x, kl->co.y, kl->co.z));
    const float area = kernel_data.integrator.falcon_photon_sun_sizex *
                       kernel_data.integrator.falcon_photon_sun_sizey;
    return watts * fabsf(nd.z) * area;
  }
  return 0.0f; /* 背景など、光子を撒かない灯 */
}

/* 1パスの時の「光子1個が運ぶ出力の目安」= 全部の灯の合計出力。
 * 灯ごとの焼きでは 1.0(ホストの falcon_photon_flux に既に出力が入っている)。
 * ★これが要るのは飛び値の頭打ち(shade_surface の 4 倍クランプ)のため。
 *   1パスではホストが渡す flux が「1 W あたり」なので、頭打ちも同じだけ
 *   小さくなってしまい、**集光が 1/250 に潰れていた**(2026-09-06 実測:
 *   自動モードの芯が蓄積の 0.014 倍)。合計は光子によらない定数なので、
 *   ここで数え直しても偏りは入らない。 */
ccl_device_inline float falcon_lt_power_scale(KernelGlobals kg)
{
  if (!kernel_data.integrator.falcon_photon_all_lights ||
      kernel_data.integrator.falcon_photon_is_world)
  {
    return 1.0f;
  }
  const int nl = kernel_data.integrator.num_lights;
  float total = 0.0f;
  for (int i = 0; i < nl; i++) {
    total += falcon_photon_light_power(kg, &kernel_data_fetch(lights, i));
  }
                                           ...
                                           const float3 diff)
{
  ...
  /* ★2026-09-06 S1: 打ち消す相手は「撃った光子の数」であって「レンダーの
   * SPP」ではない。falcon_lt_photons(画素あたりの光子数)が入っていれば
   * それを使う。今の呼び出しでは FALCON_PHOTON_N = W*H*SPP なので値は
   * SPP と厳密に同じ(画素一致)。0 のときだけ旧来の SPP に落ちる。 */
  const float ppp = kernel_data.integrator.falcon_lt_photons;
  const float samples = (ppp > 0.0f) ? ppp :
                                       (float)kernel_data.integrator.falcon_lt_samples;
