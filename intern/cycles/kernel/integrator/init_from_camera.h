                                            ...
                                            const int scheduled_sample)
{
  ...
  if (kernel_data.integrator.falcon_photon_pass) {
    ...
    /* ★2026-09-06 S4: 1回のレンダーで全部の灯から撒く。
     * 灯は出力に比例して選ぶ。選ぶ確率が w_j/Σw なので、光子1個が運ぶのは
     * Σw / N ―― どの灯を引いても同じ値になり、灯ごとに焼いて足したのと
     * 総エネルギーが一致する(門 G4)。ホストから来るのは gain/N だけ。 */
    int fl_li = 0;
    const bool fl_multi = (kernel_data.integrator.falcon_photon_all_lights != 0) &&
                          (kernel_data.integrator.falcon_photon_is_world == 0);
    if (fl_multi) {
      ...
      const float u = hash_uint3_to_float(x, y, 0x27d4eb2du ^ sample) * total;
      float acc = 0.0f;
      for (int i = 0; i < nl; i++) {
        const float pw = falcon_photon_light_power(kg, &kernel_data_fetch(lights, i));
        if (!(pw > 0.0f)) {
          continue;
        }
        fl_li = i;
    ...
    /* ★ワールドのパスはランプを全部隠して走るので lights[] が空になり得る。
     * 元の形では POINT の枝が is_world の後ろにあって評価されなかった。 */
    const int fl_type = (kernel_data.integrator.num_lights > 0) ?
                            kernel_data_fetch(lights, fl_li).type :
                            LIGHT_BACKGROUND;
    const bool fl_is_sun = fl_multi ? (fl_type == LIGHT_SUN) :
                                      (kernel_data.integrator.falcon_photon_is_sun != 0);
    const bool fl_is_spot = fl_multi ? (fl_type == LIGHT_SPOT) :
                                       (kernel_data.integrator.falcon_photon_is_spot != 0);
    if (fl_is_sun) {
      const ccl_global KernelLight *klight = &kernel_data_fetch(lights, fl_li);
      pray.P = make_float3(
      ...
      float u = rd.x, v = rd.y;
      /* ★1パスでは sun_* に太陽の発射矩形が乗っているので、窓として
       * 読んではいけない(誘導タイルは灯ごとの焼きでしか使わない)。 */
      const float win_sx = fl_multi ? 0.0f : kernel_data.integrator.falcon_photon_sun_sizex;
      if (win_sx > 0.0f) {
       ...
       * 残っていた(芯 1.326 -> 1.010 の件、(internal notes))。
