...
Upstream: https://github.com/usrname0/BL_EasyCrop (usrname0, GPL-3.0-or-later).
Bundled as-is aside from this `bl_info` shim: this build's `scripts/addons_core`
loader is the legacy one and skips any add-on without `bl_info` (upstream ships
only `blender_manifest.toml`, the newer extensions-repository format).
"""
...
bl_info = {
    ...
    "version": (2, 1, 0),
    "blender": (4, 4, 0),
    "location": "Sequencer Preview > Shift+C",
    "description": "Click-and-drag handles to crop strips in the VSE Preview window",
    "doc_url": "https://github.com/usrname0/BL_EasyCrop",
    "category": "Sequencer",
}
...
def menu_func_crop(self, context):
    ...
    if context.space_data.view_type in {'PREVIEW', 'SEQUENCER_PREVIEW'}:
        ...
        self.layout.operator("sequencer.easycrop_crop", text="Crop")
...
def register():
    ...
    if keyconfig:
        ...
        addon_keymaps.append(
            (keymap, keymap.keymap_items.new("sequencer.easycrop_crop", 'C', 'PRESS', shift=True)))
        addon_keymaps.append(
