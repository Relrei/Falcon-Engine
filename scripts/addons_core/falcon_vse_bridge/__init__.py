...
def _jump_to_video_editing():
    try:
        ...
        for scene in bpy.data.scenes:
            ...
            if engine == FALCON_VSE_ENGINE and _engine_seen.get(key) != FALCON_VSE_ENGINE:
                changed.add(key)
...
class FALCON_VSE_OT_fit_frame_range(Operator):
    ...
    def execute(self, context):
        ...
        if scene is None:
            self.report({'WARNING'}, "Sequencer のシーンがありません")
            return {'CANCELLED'}
        fitted = _fit_frame_range(scene)
        if fitted is None:
            # ★空なら何もしない(範囲を 1-1 に潰さない)。
            self.report({'WARNING'}, "ストリップがありません")
            return {'CANCELLED'}
...
def _on_load_post(*args):
    ...
    if clear_stuck_sequencer_on_load():
        ...
        if stuck:
            ...
            print("falcon_vse_bridge: 古い .blend の use_sequencer を落とした (%s)。"
                  "戻すには FALCON_VSE_CLEAR_STUCK_SEQUENCER=0" % ", ".join(stuck))
    ...
    if cleared:
        # ★黙って倒さない。何をしたかは端末に1行残す。
        print("falcon_vse_bridge: プレビュー範囲の旗を落とした (%s)。"
              "戻すには FALCON_VSE_CLEAR_PREVIEW_RANGE=0" % ", ".join(cleared))
...
#   **元の場所から黙って消える**(内部の記録)。門でも「借り元が生きていること」を見る。
...
OUTPUT_MOVED_TEXT = "VSE では出力の項目もレンダーの欄にまとめています"


class FALCON_VSE_RenderEngine(RenderEngine):
    """VSE(項目だけ。絵は Sequencer が書く)"""
...
def sequencer_cache_rows(scene):
    ...
    if limit:
        rows.append(("label", "太らせない下限: %d MB (FALCON_VSE_MEM_SOFT_MB)" % limit))
    else:
        rows.append(("label", "太らせない下限: 切 (FALCON_VSE_MEM_SOFT_MB=0)"))
    return rows
...
def encoding_status_rows(scene):
    ...
    """
    if fastpath_enabled():
        return [("label", "切っただけの経路: 入")]
    return [("label", "切っただけの経路: 切 (FALCON_VSE_FASTPATH=0)")]
